function Fric(keyInput) {
    ...
    async function makeRequest(downloadAssets = false) {
        ...
        for (let i = 0; i < fractalsArray.length; i++) {
            console.log(`https://dsqdzkf7h2.execute-api.eu-central-1.amazonaws.com/prod/fractal${fractalsArray[i].GetQuery()}&token=${key}`);
    ...
    return {
        AddFractal, AddFractals, makeRequest, load, show, GetResult
    }
...
function Fractal(data = '') {
     ...
     */
    function GetFormat() {
        if (optional.hasOwnProperty('format'))
    ...
    function GetOptional() {
        let queryOptional = '';
        const optionalParams = ['lineWidth', 'antialias', 'lengthFactor', 'lineLength', 'format'];
        ...
        if (optional.hasOwnProperty('backColor')) {
            queryOptional += `&backColor=${encodeURIComponent(optional['backColor'])}`;
        }
        if (optional.hasOwnProperty('lineColor')) {
            queryOptional += `&lineColor=${encodeURIComponent(optional['lineColor'])}`;
        }
        if (optional.hasOwnProperty('stringify')) {
    ...
    function GetQuery() {
        validateOptional();
        return `?axiom=${encodeURIComponent(GetAxiom())}&angle=${GetAngle()}&rules=${encodeURIComponent(GetRules().join(','))}&depth=${GetDepth()}&${GetOptional()}`;
    }

    return {
        Completed, AddRule, SetAxiom, SetAngle, SetDepth, SetFormat, GetQuery, OptionalParameters, GetFormat
    }
}
